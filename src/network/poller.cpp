...
namespace netsys
{
    ...
    Poller::Result Poller::poll(const int &timeout_ms)
    {
        ...
        for(unsigned int i = 0; i < actions_.size(); i++)
        {
            ...
            pollfds_.at(i).events = (actions_.at(i).active and actions_.at(i).when_interested()) ? actions_.at(i).direction : 0;

            if(actions_.at(i).direction == Direction::In and actions_.at(i).fd.eof())
                pollfds_.at(i).events = 0;
        }

        if(not accumulate(pollfds_.begin(), pollfds_.end(), false, [](bool acc, pollfd x){ return acc or x.events; }))
